...
async function fetchLatestEurostatValue(dataset, params, countries) {
...
export function registerCompareEconomiesTool(server) {
    server.tool('compare_eu_economies', 'Compare key economic indicators (inflation, GDP growth, unemployment) across multiple EU countries side by side. Returns a unified snapshot per country.', {
        countries: z
        ...
        return withMcpMiddleware({ serverName: SERVER_NAME, toolName: 'compare_eu_economies' }, async () => {
            ...
            const cacheKey = `compare_eu_economies:${hashParams(params)}`;
            ...
            const snapshots = countries.map((code) => ({
...
//# sourceMappingURL=compare-economies.js.map